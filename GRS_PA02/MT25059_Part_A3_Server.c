...
void *handle_client(void *arg) {
    ...
    int opt = 1;
    if (setsockopt(client_sock, SOL_SOCKET, SO_ZEROCOPY, &opt, sizeof(opt))) {
    ...
    while (1) {
        ssize_t sent = sendmsg(client_sock, &message_header, MSG_ZEROCOPY | MSG_NOSIGNAL); 
        
        if (sent == -1 && errno == ENOBUFS) {
            continue; 
        }
        if (sent <= 0) break;
        ...
        char control_err[100];
        
        memset(&msg_err, 0, sizeof(msg_err));
        msg_err.msg_iov = &iov_err;
        msg_err.msg_iovlen = 1;
        msg_err.msg_control = control_err;
        msg_err.msg_controllen = sizeof(control_err);
...
int main() {
    ...
    printf("A3 (Zero-Copy) Server listening on port %d...\n", PORT);

    while (1) {
        new_sock = malloc(sizeof(int));
        if ((*new_sock = accept(server_fd, (struct sockaddr *)&address, (socklen_t*)&addrlen)) < 0) {
            perror("accept"); free(new_sock); continue;
        }
        pthread_t thread_id;
