...
echo "Program+Function,CPU_Avg_%,Mem_Max_KB,kB_Wrtn_Total,Duration_Sec" > $OUTPUT_FILE
...
monitor_process() {
    ...
    while kill -0 $main_pid 2>/dev/null; do
        ...
        if (( $(echo "$m > $mem_max" | bc -l) )); then mem_max=$m; fi
...
for prog in "${PROGRAMS[@]}"; do
    for worker in "${WORKERS[@]}"; do
        ...
        echo "Finished ${prog}+${worker}. Time: ${duration}s"
        echo "--------------------------------------"
        rm -f inOut_*.dat
        sleep 1
    done
